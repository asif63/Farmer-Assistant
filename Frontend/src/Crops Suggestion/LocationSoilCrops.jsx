...
const LocationSoilCrops = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      if (!soil || !locationName) {
        setErrorMessage("Please provide correct data");
        return;
      }

      const response = await axios.get("http://localhost:3010/crops", {
        params: { soil, location: locationName },
      });
...
export default LocationSoilCrops;
