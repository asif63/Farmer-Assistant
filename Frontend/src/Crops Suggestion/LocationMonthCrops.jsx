...
const LocationMonthCrops = () => {
  ...
  const fetchData = async () => {
    try {
      if (!month || !locationName) {
        setErrorMessage("Please provide correct data");
        return;
      }

      const response = await axios.get("http://localhost:3007/crops", {
        params: { month,  location: locationName },
      });
...
export default LocationMonthCrops;
